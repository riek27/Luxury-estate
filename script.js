...
// portfolio-minimal.js - Fix for number display issues

document.addEventListener('DOMContentLoaded', function() {
    // Fix 1: Counter animation - prevent 0 from showing initially
    const statNumbers = document.querySelectorAll('.stat-number');
    statNumbers.forEach(stat => {
        // Hide the number initially until animation starts
    ...
    // Fix 2: Portfolio item count display
    const portfolioItems = document.querySelectorAll('.portfolio-item, .gallery-item');
    const itemCountElements = document.querySelectorAll('.item-count, .project-count');
    ...
    // Fix 3: Lightbox navigation (remove 0-based index display)
    const lightbox = document.getElementById('lightbox');
    if (lightbox) {
        // Remove any index display that shows "0"
        const indexDisplays = lightbox.querySelectorAll('.lightbox-index, .image-count');
        indexDisplays.forEach(display => {
            if (display.textContent.includes('0') || display.textContent === '0') {
